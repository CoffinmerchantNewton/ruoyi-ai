...
@Slf4j
@Service
public class ChatGraphLLMServiceImpl implements IGraphLLMService {

    @Override
    public String extractGraph(String prompt, ChatModelVo chatModel) {
        log.info("chat模型调用: model={}, apiHost={}, 提示词长度={}",
                chatModel.getModelName(), chatModel.getApiHost(), prompt.length());

        try {
            //如果apihost以v1/chat/completions结尾则去掉
            if (chatModel.getApiHost().endsWith("v1/chat/completions")) {
                chatModel.setApiHost(chatModel.getApiHost().replace("v1/chat/completions", ""));
            }
            // 创建 ChatStreamClient
            ...
            log.info("chat模型响应成功: 耗时={}ms, 响应长度={}", duration, responseText.length());

            return responseText;

        } catch (Exception e) {
            log.error("chat模型调用失败: {}", e.getMessage(), e);
            throw new RuntimeException("chat模型调用失败: " + e.getMessage(), e);
        }
    }

    @Override
    public String getCategory() {
        return "chat";  // 对应 ChatModel 表中的 category 字段
    }
}
