...
import org.ruoyi.dingtalk.vo.ContactUser;
...
public class DingTalkLoginService {
    ...
    public LoginVo dingTalkLogin(String unionId) {
        ...
        // 使用 unionId 查询绑定用户，如未绑定用户则根据业务自行处理，例如创建默认用户
        SysUserVo user = userService.selectUserByUnionId(unionId);
        VisitorLoginUser loginUser = new VisitorLoginUser();

        if (ObjectUtil.isNull(user)) {
            SysUserBo sysUser = new SysUserBo();
            String name = dingTalkUser.getName() != null ? dingTalkUser.getName() : "用户" + UUID.randomUUID().toString().replace("-", "");
            // 设置默认用户名
